...
def load_spectrogram(file_path, target_length=430):
...
class Encoder(nn.Module):
    def __init__(self, input_dim, latent_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(input_dim, 430),
            nn.ReLU(),
            nn.Linear(430, latent_dim)
        )
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, output_dim):
        ...
        self.fc = nn.Sequential(
            nn.Linear(latent_dim, 430),
            nn.ReLU(),
            nn.Linear(430, output_dim)
        )

    def forward(self, x):
        return self.fc(x).reshape(-1, 128, 430)  
...
data_path = r"C:\Users\rahul\OneDrive\Documents\DS_ML projects\merged_datasetv3.csv" # the spreadsheet provided in zip file 
