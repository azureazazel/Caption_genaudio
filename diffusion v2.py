...
def train_diffusion_model(model, dataloader, timesteps, epochs, lr):
    ...
    for epoch in range(epochs):
        ...
        for caption_embeddings, spectrograms in dataloader:
            ...
            noisy_spectrogram, noise = add_noise(spectrograms, t, beta_schedule)  

            optimizer.zero_grad()
            predicted_noise = model(noisy_spectrogram, caption_embeddings) 
            loss = loss_fn(predicted_noise, noise) 
            loss.backward()
            optimizer.step()
...
def spectrogram_to_audio_griffinlim(spectrogram, sr=22050, n_iter=32, n_fft=1024, hop_length=256):
    ...
    spectrogram = np.clip(spectrogram, a_min=1e-10, a_max=None)  
...
beta_schedule = get_beta_schedule(timesteps).to(device)
noisy_input = torch.randn(1, 1, 128, 430).to(device)  
...
spectrogram = generated_spectrogram.squeeze(0)  
